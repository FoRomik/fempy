...
class Genesis(object):
    ...
    def put_init(self, title, num_dim, num_nodes, num_elem, num_elem_blk,
        ...
        def create_and_alloc(num, dim_id, var_id, var_stat, var_name, nz=0):
            if not num:
                return

            # block/set meta data
            self.db.createDimension(dim_id, num)

            if nz: prop1 = np.arange(num, dtype=np.int32)
            else: prop1 = np.zeros(num, dtype=np.int32)
            self.db.createVariable(var_id, DTYPE_INT, (dim_id,))
            ...
            self.db.createVariable(var_stat, DTYPE_INT, (dim_id,))
            self.db.variables[var_stat][:] = status

            names = np.array(["\0" for _ in prop1])
            self.db.createVariable(var_name, DTYPE_TXT, (dim_id, DIM_STR))
            for i in range(num):
                self.db.variables[var_name][i, :] = "\0"
        ...
        for i in range(num_dim):
            self.db.variables[VAR_NAME_COOR][i, :] = "\0"
...
#        self.db.createDimension(DIM_NUM_EM, 1)
        ...
        self.db.createVariable(VAR_MAP, DTYPE_INT, (DIM_NUM_ELEM,))
    ...
    def put_elem_block(self, elem_blk_id, elem_type, num_elem_this_blk,
        ...
        if num_attr:
            ...
            self.db.variables[VAR_NAME_ATTRIB(i+1)][:] = "\0"
